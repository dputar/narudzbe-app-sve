                            ...
                            for i in range(0, len(df_upload), batch_size):
                                ...
                                for _, row in batch.iterrows():
                                    ...
                                    if not broj_narudzbe:
                                        broj_praznih += 1
                                        st.write("Preskočen red: nema broj_narudzbe")
                                        continue
                                    if broj_narudzbe in postojeći_brojevi:
                                        broj_duplikata += 1
                                        st.write(f"Preskočen duplikat: {broj_narudzbe}")
                                        continue
                                    novi = {
                                    ...
                                    broj_dodanih += 1
                                    st.write(f"Dodan red: {broj_narudzbe}")
        ...
        if uploaded_file:
            try:
                ...
                if st.button("Učitaj sve u bazu (batch po 500)", type="primary"):
                    ...
                    st.write(f"Broj postojećih naziva u bazi: {len(postojeći_nazivi)}")
                    ...
                    for i in range(0, len(df_upload), batch_size):
                        ...
                        for idx, row in batch.iterrows():
                            naziv_raw = row.get("NAZIV")
                            naziv = str(naziv_raw).strip() if naziv_raw is not None else ""

                            if not naziv:
                                broj_praznih += 1
                                st.write(f"Preskočen red {idx}: naziv je prazan ili None")
                                continue

                            st.write(f"Provjeravam naziv: '{naziv}'")

                            # Zakomentiraj sljedeće 3 linije da se DODAJU SVI REDOVI (čak i duplikati)
                            # if naziv.lower() in postojeći_nazivi:
                            #     broj_duplikata += 1
                            #     st.write(f"Preskočen duplikat: '{naziv}'")
                            #     continue
                            ...
                            try:
                                ...
                                cijena = 0
                                broj_nevaljanih += 1
                                st.write(f"Nevaljana cijena u redu {idx}: '{cijena_raw}' → postavljeno na 0")
                            ...
                            try:
                                ...
                                st.write(f"Uspješno dodan: '{naziv}'")
                            except Exception as insert_e:
                                st.error(f"Greška pri insertu reda {idx} ('{naziv}'): {insert_e}")
                ...
                st.error(f"Greška pri čitanju Excela: {e}")
                st.error("Provjeri format datoteke i stupce (npr. 'NAZIV' mora postojati i biti popunjen).")
