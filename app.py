...
from datetime import timedelta
from datetime import date
import bcrypt  # Dodano za hashiranje lozinki

st.set_page_config(page_title="Sustav narudžbi", layout="wide")

# Supabase konekcija
SUPABASE_URL = "https://vwekjvazuexwoglxq...(truncated)"
SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZ3ZWtqdmF6dWV4d29nbHhxdW5jIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MDg4OTc1NzcsImV4cCI6MjAyNDQ3MzU3N30.XZ1QhW1o9R9i5Q5y2L5q1fD3i5kYdP7h3jV2JqKq3i0"
...
# Funkcija za autentifikaciju iz tablice korisnika
...
if st.session_state.stranica == "login":
    ...
    st.sidebar.title(f"Dobro došli, {st.session_state.user['ime_prezime']}")
...
if st.session_state.stranica == "main_orders":
    ...
    # Dohvati dobavljače
    try:
        dobavljači_response = supabase.table("dobavljači").select("id, naziv").execute()
        dobavljači = dobavljači_response.data or []
        dobavljači_options = {d["naziv"]: d["id"] for d in dobavljači}
    except Exception as e:
        st.error(f"Greška pri dohvaćanju dobavljača: {str(e)}")
        dobavljači_options = {}

    # Forma za dodavanje narudžbe
    with st.form("dodaj_narudžbu_form", clear_on_submit=True):
        st.subheader("Dodaj novu narudžbu")

        dobavljač_id = st.selectbox("Dobavljač *", list(dobavljači_options.keys()), index=0, key="narudžba_dobavljač")
        datum_narudžbe = st.date_input("Datum narudžbe *", value=datetime.now(TZ).date(), key="narudžba_datum")
        napomena = st.text_area("Napomena (opcionalno)", key="narudžba_napomena")

        submitted = st.form_submit_button("Dodaj narudžbu", type="primary")

    if submitted:
        if not dobavljač_id:
            st.error("Dobavljač je obavezan!")
        elif not datum_narudžbe:
            st.error("Datum narudžbe je obavezan!")
        else:
            try:
                novi = {
                    "dobavljač_id": dobavljači_options[dobavljač_id],
                    "datum_narudžbe": datum_narudžbe.isoformat(),
                ...
                st.error(f"Greška pri dodavanju narudžbe: {str(e)}")

    # Prikaz narudžbi
    st.subheader("Sve narudžbe")
    try:
        orders_response = supabase.table("main_orders")\
            .select("*, dobavljači!inner(naziv)")\
            .order("datum_narudžbe", desc=True)\
            .execute()

        df_orders = pd.DataFrame(orders_response.data or [])

        if not df_orders.empty:
            df_orders["dobavljač_naziv"] = df_orders["dobavljači"].apply(lambda x: x["naziv"] if isinstance(x, dict) and "naziv" in x else "Nepoznato")
            df_orders = df_orders.drop(columns=["dobavljači"])

            df_orders["Obriši"] = False

            edited_df = st.data_editor(
                df_orders[["id", "dobavljač_naziv", "datum_narudžbe", "napomena", "unio_korisnik", "created_at", "Obriši"]],
                column_config={
                    "id": st.column_config.NumberColumn("ID", disabled=True),
                    "dobavljač_naziv": st.column_config.TextColumn("Dobavljač", disabled=True),
            ...
            if st.button("Spremi izmjene i obriši označene"):
                ...
                for idx, row in edited_df.iterrows():
                    original_row = df_orders.loc[idx]
                    ...
                    changed_fields = {}
                    for field in ["datum_narudžbe", "napomena"]:
                        if row[field] != original_row[field]:
                            changed_fields[field] = row[field]

                    if changed_fields:
                        supabase.table("main_orders").update(changed_fields).eq("id", row["id"]).execute()

                if to_delete:
                    for rec_id in to_delete:
                        supabase.table("main_orders").delete().eq("id", rec_id).execute()
        ...
        st.error(f"Greška pri dohvaćanju/uređivanju narudžbi: {str(e)}")
    ...
    with st.form("dodaj_proizvod_form", clear_on_submit=True):
        ...
        naziv = st.text_input("Naziv proizvoda *", key="proizvod_naziv")
        cijena = st.number_input("Cijena *", min_value=0.0, step=0.01, key="proizvod_cijena")
        opis = st.text_area("Opis (opcionalno)", key="proizvod_opis")

        submitted = st.form_submit_button("Dodaj proizvod", type="primary")

    if submitted:
        if not naziv:
            st.error("Naziv proizvoda je obavezan!")
        elif cijena <= 0:
            st.error("Cijena mora biti veća od 0!")
        else:
            try:
                novi = {
                    "naziv": naziv.strip(),
                ...
                st.error(f"Greška pri dodavanju proizvoda: {str(e)}")

    # Prikaz proizvoda
    st.subheader("Svi proizvodi")
    try:
        proizvodi_response = supabase.table("proizvodi")\
            .select("*")\
        ...
        if not df_proizvodi.empty:
            df_proizvodi["Obriši"] = False

            edited_df = st.data_editor(
                df_proizvodi[["id", "naziv", "cijena", "opis", "created_at", "Obriši"]],
                column_config={
                    "id": st.column_config.NumberColumn("ID", disabled=True),
                    "created_at": st.column_config.TextColumn("Kreirano", disabled=True),
                    "Obriši": st.column_config.CheckboxColumn("Obriši", default=False)
                ...
                use_container_width=True,
                num_rows="fixed",
                key="proizvodi_editor"
            )

            if st.button("Spremi izmjene i obriši označene"):
                to_delete = []
                for idx, row in edited_df.iterrows():
                    original_row = df_proizvodi.loc[idx]
                    ...
                    changed_fields = {}
                    for field in ["naziv", "cijena", "opis"]:
                        if row[field] != original_row[field]:
                            changed_fields[field] = row[field]

                    if changed_fields:
                        supabase.table("proizvodi").update(changed_fields).eq("id", row["id"]).execute()

                if to_delete:
                    for rec_id in to_delete:
                        supabase.table("proizvodi").delete().eq("id", rec_id).execute()
        ...
        st.error(f"Greška pri dohvaćanju/uređivanju proizvoda: {str(e)}")

elif st.session_state.stranica == "dobavljači":
    st.title("🛒 Dobavljači")

    # Forma za dodavanje dobavljača
    with st.form("dodaj_dobavljača_form", clear_on_submit=True):
        st.subheader("Dodaj novog dobavljača")

        naziv = st.text_input("Naziv dobavljača *", key="dobavljač_naziv")
        adresa = st.text_input("Adresa (opcionalno)", key="dobavljač_adresa")
        kontakt = st.text_input("Kontakt (opcionalno)", key="dobavljač_kontakt")

        submitted = st.form_submit_button("Dodaj dobavljača", type="primary")
    ...
    if submitted:
        if not naziv:
            ...
            try:
                novi = {
                    "naziv": naziv.strip(),
                    "adresa": adresa.strip() or None,
                    "kontakt": kontakt.strip() or None,
                    "created_at": datetime.now(TZ).isoformat()
                }
                supabase.table("dobavljači").insert(novi).execute()
                st.success("Dobavljač dodan!")
                st.rerun()
            except Exception as e:
                st.error(f"Greška pri dodavanju dobavljača: {str(e)}")

    # Prikaz dobavljača
    st.subheader("Svi dobavljači")
    try:
        dobavljači_response = supabase.table("dobavljači")\
            .select("*")\
            .order("created_at", desc=True)\
            .execute()

        df_dobavljači = pd.DataFrame(dobavljači_response.data or [])

        if not df_dobavljači.empty:
            df_dobavljači["Obriši"] = False

            edited_df = st.data_editor(
                df_dobavljači[["id", "naziv", "adresa", "kontakt", "created_at", "Obriši"]],
                column_config={
                    "id": st.column_config.NumberColumn("ID", disabled=True),
                    "created_at": st.column_config.TextColumn("Kreirano", disabled=True),
                    "Obriši": st.column_config.CheckboxColumn("Obriši", default=False)
                ...
                use_container_width=True,
                num_rows="fixed",
                key="dobavljači_editor"
            )

            if st.button("Spremi izmjene i obriši označene"):
                to_delete = []
                for idx, row in edited_df.iterrows():
                    original_row = df_dobavljači.loc[idx]
                    ...
                    changed_fields = {}
                    for field in ["naziv", "adresa", "kontakt"]:
                        if row[field] != original_row[field]:
                            changed_fields[field] = row[field]

                    if changed_fields:
                        supabase.table("dobavljači").update(changed_fields).eq("id", row["id"]).execute()

                if to_delete:
                    for rec_id in to_delete:
                        supabase.table("dobavljači").delete().eq("id", rec_id).execute()

                st.success("Izmjene i brisanja spremljeni!")
                st.rerun()

        else:
            st.info("Još nema dobavljača.")
    except Exception as e:
        st.error(f"Greška pri dohvaćanju/uređivanju dobavljača: {str(e)}")

elif st.session_state.stranica == "korisnici":
    st.title("👥 Korisnici")

    # Forma za dodavanje korisnika
    with st.form("dodaj_korisnika_form", clear_on_submit=True):
        st.subheader("Dodaj novog korisnika")

        ime_prezime = st.text_input("Ime i prezime *", key="korisnik_ime_prezime")
        korisničko_ime = st.text_input("Korisničko ime *", key="korisnik_korisničko_ime")
        lozinka = st.text_input("Lozinka *", type="password", key="korisnik_lozinka")
        tip_korisnika = st.selectbox("Tip korisnika *", ["korisnik", "administrator"], index=0, key="korisnik_tip")
        aktivan = st.checkbox("Aktivan", value=True, key="korisnik_aktivan")
        godisnji_dani = st.number_input("Godišnji dani", min_value=0, value=20, key="korisnik_godisnji_dani")
        slobodni_dani = st.number_input("Slobodni dani", min_value=0, value=0, key="korisnik_slobodni_dani")

        submitted = st.form_submit_button("Dodaj korisnika", type="primary")

    if submitted:
        if not ime_prezime or not korisničko_ime or not lozinka:
            st.error("Svi obavezni polja moraju biti popunjena!")
        else:
            try:
                hashed_lozinka = bcrypt.hashpw(lozinka.encode('utf-8'), bcrypt.gensalt())
                novi = {
                    "ime_prezime": ime_prezime.strip(),
                    "korisničko_ime": korisničko_ime.strip(),
                    "lozinka": hashed_lozinka.decode('utf-8'),
                    "tip_korisnika": tip_korisnika,
                    "aktivan": aktivan,
                    "godisnji_dani": godisnji_dani,
                    "slobodni_dani": slobodni_dani,
    ...
    try:
        korisnici_response = supabase.table("korisnici")\
            .select("*")\
            .order("created_at", desc=True)\
            .execute()

        df_korisnici = pd.DataFrame(korisnici_response.data or [])

        if not df_korisnici.empty:
            df_korisnici["Obriši"] = False

            edited_df = st.data_editor(
                df_korisnici[["id", "ime_prezime", "korisničko_ime", "tip_korisnika", "aktivan", "godisnji_dani", "slobodni_dani", "created_at", "Obriši"]],
                column_config={
                    "id": st.column_config.NumberColumn("ID", disabled=True),
                    "created_at": st.column_config.TextColumn("Kreirano", disabled=True),
                    "Obriši": st.column_config.CheckboxColumn("Obriši", default=False)
            ...
            if st.button("Spremi izmjene i obriši označene"):
                to_delete = []
                for idx, row in edited_df.iterrows():
                    original_row = df_korisnici.loc[idx]
                    ...
                    changed_fields = {}
                    for field in ["ime_prezime", "korisničko_ime", "tip_korisnika", "aktivan", "godisnji_dani", "slobodni_dani"]:
                        if row[field] != original_row[field]:
                            changed_fields[field] = row[field]

                    if "lozinka" in row and row["lozinka"] != original_row["lozinka"]:
                        hashed_lozinka = bcrypt.hashpw(row["lozinka"].encode('utf-8'), bcrypt.gensalt())
                        changed_fields["lozinka"] = hashed_lozinka.decode('utf-8')

                    if changed_fields:
                        supabase.table("korisnici").update(changed_fields).eq("id", row["id"]).execute()

                if to_delete:
                    for rec_id in to_delete:
                        supabase.table("korisnici").delete().eq("id", rec_id).execute()

                st.success("Izmjene i brisanja spremljeni!")
                st.rerun()

        else:
            st.info("Još nema korisnika.")
    except Exception as e:
        st.error(f"Greška pri dohvaćanju/uređivanju korisnika: {str(e)}")

elif st.session_state.stranica == "dokumenti":
    # ... (cijeli kod za dokumenti, koji je već finalni – ostavi netaknut)
    # (nisam ga kopirao ovdje da ne bude predugo, ali u tvom app.py ostaje isti)

# Automatski refresh svakih 5 minuta
